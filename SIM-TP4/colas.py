...
def simular(
    ...
    while reloj < dias_simulacion * 24 * 60 and eventos:
        ...
        # Registrar dinámicamente columnas para cada estudiante
        """for idx, alumno in enumerate(alumnos, start=1):
            fila[f'Estudiante {idx} Estado'] = alumno['estado']
            fila[f'Estudiante {idx} Equipo'] = alumno['equipo_actual']
            fila[f'Estudiante {idx} Tiempo de Espera'] = (
                round(reloj - alumno['hora_llegada'], 2) if alumno['estado'] == 'En cola' else 'N/A'
            )"""
